...
@Component({
  selector: "app-post-view-detail-user",
  templateUrl: "./post-view-detail-user.component.html",
  styleUrls: ["./post-view-detail-user.component.css"],
  providers: [
    ViewPostServiceProxy,
    ManageAppointmentSchedulesServiceProxy,
    UserCommentServiceProxy,
...
export class PostViewDetailUserComponent
  ...
  implements OnInit
{
  ...
  ngOnInit() {
    ...
    this._commentsService.updateComment.subscribe(
      (updatedComment: UserCommentDto) => {
    ...
    this._commentsService.deleteComment.subscribe(
    ...
    this._commentsService.allCommentsReceived.subscribe(
      (cmt: UserCommentViewDto[]) => {
        cmt.forEach((comment) => {
          // Chuyển đổi creationTime thành đối tượng moment
          const creationTimeMoment = moment(comment.creationTime);

          const timeDiff = Math.abs(
            new Date().getTime() - creationTimeMoment.toDate().getTime()
          );
  ...
  getStatus(): void {
    this._postService.statusRoomLike(this.postId).subscribe((res) => {
      this.statusLike = res;
    });
  }
  ...
  getStatusSchedule(): void {
    this._postScheduleService.statusSchedule(this.post).subscribe((res) => {
  ...
  showPostLocation(): void {
    if (this.post.address + ", " + this.post.district + ", " + this.post.city) {
      this.getLocationForPost(
        this.post.address + ", " + this.post.district + ", " + this.post.city
      );
    } else {
  ...
  booking(postsId: number): void {
    if (!this.showUserId) {
      this.notify.warn("Bạn cần đăng nhập vào hệ thống !");
      return;
    }
